...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 1. Initialize Cloudinary (Necessary for your images)
        val config = mapOf("cloud_name" to "djtr5luf6", "secure" to true)
        try {
            MediaManager.init(this, config)
        } catch (e: Exception) {
            // Initialization already handled or failed silently
        }

        setContent {
            InventoryProTheme {
                // 2. Launch the App Navigation
                // All the UpdateManager logic is now removed for a smoother experience
                AppNavHost()
            }
        }
    }
